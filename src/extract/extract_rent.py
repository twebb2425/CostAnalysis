...
def get_latest_acs_year():
    ...
    raise RuntimeError("No recent ACS 5-year dataset could be found.")
...
def extract_housing_data():
    ...
    logger.info(f"Latest ACS 5-year dataset found: {census_year}")
    ...
    logger.info(f"Raw housing data saved to: {OUTPUT_PATH}")

    # Returns both the extracted DataFrame and its Census year for reuse elsewhere.
    return rent_df, census_year
